...
function LeaveRequestCreate() {
  const [leaveRequets, setLeaveRequets] = useState<LeaveType[]>([]);
  ...
  useEffect(() => {
    defaultAxios
      .get("http://127.0.0.1:8000/api/v1/leave-types?is_active=1")
      .then((res) => setLeaveRequets(res.data.data))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 422) {
        ...
        // navigate("/panel/leave-request", {
        //   state: { message: "Failed to create leave request", status: "error" },
        // });
    ...
    <div className="w-full mx-auto bg-white p-6 rounded-xl shadow-md mt-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          {errors["additional_info.replacement_person"] && (
            <p className="text-sm text-red-500 mt-1">
              {errors["additional_info.replacement_person"][0]}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors["additional_info.delegated_task"] && (
            <p className="text-sm text-red-500 mt-1">
              {errors["additional_info.delegated_task"][0]}
            </p>
          )}
        </div>
        ...
        <div className="flex justify-between">
          ...
          />
          <Button text="Save Request" color="primary" type="submit" />
        </div>
      </form>
    </div>
  );
