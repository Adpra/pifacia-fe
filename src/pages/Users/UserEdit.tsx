...
function UserEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await defaultAxios.put(`http://127.0.0.1:8000/api/v1/users/${id}`, data);
      navigate("/panel/user", {
        state: { message: "User successfully updated", status: "success" },
      });
    } catch (error) {
      ...
      if (err.response?.status === 422) {
        ...
        navigate("/panel/user", {
          state: { message: "Failed to update user", status: "error" },
        });
      }
    } finally {
      setLoading(false);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            onChange={(val) => setData({ ...data, role_id: val })}
          />
          {errors.role_id && (
        ...
        <div className="flex justify-between">
          ...
          />
          <Button text="Update User" color="primary" type="submit" />
        </div>
      </form>
    </div>
  );
